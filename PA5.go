...
/* LOOPING SERVER
You might be thinking already. Isn’t a server always on and ready for client requests
any time? The PA4.go however accepts only a file upload and then quits. You are
right. PA4.go is far off a professional server. To address the issue, you need to extend
PA4.go to PA5.go where the file upload service will continue forever. More
...
func main() {
	...
	for {
		...
		response := fmt.Sprintf("Original file size: %d bytes \nNew file size: %d bytes", originalFileSize, newFileSize.Size())
		print(response)
		conn.Write([]byte(response + "\n"))

		// Step 7: Close the connection
		fmt.Printf("\nFile uploaded and processed. Connection closed.\n")

		// close connection & terminate
		conn.Close()
		// close file connection otherwise the connection remains open, since the connection close is deferred..
		// oof.
		newFile.Close()
	}
}
